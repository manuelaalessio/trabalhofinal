...
request.onload = function(){
  ...
  let destaque = new NoticiaDestaque(noticias[0].urlToImage, noticias[0].publishedAt, noticias[0].author,  noticias[0].title, noticias[0].description);

  d.insertAdjacentHTML('afterbegin', '<div id="test">' + destaque.mostrarImagem + '</div>');

  noticias.forEach(function(noticia){
    let n = new Noticia(noticia.title, noticia.publishedAt, noticia.author, noticia.description);
